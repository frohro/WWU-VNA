...
from PyQt5.QtCore import QRegExp, QTimer, QSettings, QDir, Qt, QIODevice, QByteArray
...
class VNA(QMainWindow, Ui_VNA):
  ...
  def __init__(self):
    ...
    '''rx = QRegExp('^(([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])\.){3}([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])$')      Not needed for this 
    ...
    #self.serial.connected.connect(self.connected)
    self.serial.readyRead.connect(self.read_data)
    self.serial.error.connect(self.display_error)
  ...
  def start(self):
    if self.idle:
      ...
      self.serial.setPortName(self.addrValue.text())
  ...
  def stop(self):
    self.idle = True
    self.cancel()
    #self.serial.abort()                    Not sure what it does but serial doesnt have an abort function
  ...
  '''if not self.reading:
        self.serial.readAll()
        return
      size = self.serial.bytesAvailable()
      self.progressBar.setValue((self.offset + size) / 16)
      limit = 16 * self.sweep_size
      if self.offset + size < limit:
        self.buffer[self.offset:self.offset + size] = self.serial.read(size)
        self.offset += size
      else:
        self.buffer[self.offset:limit] = self.serial.read(limit - self.offset)
        adc1 = self.data[0::2]
        adc2 = self.data[1::2]
  ...
  def display_error(self, socketError):
    ...
    if socketError == 'timeout':
      ...
      QMessageBox.information(self, 'VNA', 'Error: %s.' % self.serial.errorString())
    self.stop()

  def set_start(self, value):
    self.sweep_start = value*10**3

  def set_stop(self, value):
    self.sweep_stop = value*10**3
  ...
  def set_rate(self, value):
    ...
    self.serial.write(struct.pack('<I', 3 << 28 | int(rate)))

  def set_corr(self, value):
    if self.idle: return
    self.serial.write(struct.pack('<I', 4 << 28 | int(value)))

  def set_phase1(self, value):
    if self.idle: return
    self.serial.write(struct.pack('<I', 5 << 28 | int(value)))

  def set_phase2(self, value):
    if self.idle: return
    self.serial.write(struct.pack('<I', 6 << 28 | int(value)))

  def set_level1(self, value):
    if self.idle: return
    data = 0 if value == -90 else int(32766 * np.power(10.0, value / 20.0))
    self.serial.write(struct.pack('<I', 7 << 28 | int(data)))

  def set_level2(self, value):
    if self.idle: return
    data = 0 if value == -90 else int(32766 * np.power(10.0, value / 20.0))
    self.serial.write(struct.pack('<I', 8 << 28 | int(data)))

  def set_gpio(self, value):
    if self.idle: return
    self.serial.write(struct.pack('<I', 9 << 28 | int(value)))
  ...
  def sweep(self, mode):
    ...
    self.received_chunks = 0
    self.reading = True
    '''self.serial.write(struct.pack('<I', 0 << 28 | int(self.sweep_start * 1000)))
