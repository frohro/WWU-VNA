...
class VNA(QMainWindow, Ui_VNA):
  ...
  def start(self):
    if self.idle:
      try:
        self.serial = serial.Serial(self.addrValue.text(),115200,timeout = 3)
  ...
  def stop(self):
    self.idle = True
    self.cancel()
    '''self.socket.abort()                          This is used to abort connection to socket.... Not needed for serial???'''
  ...
  def display_error(self, socketError):
    ...
    if socketError == 'timeout':
      QMessageBox.information(self, 'VNA', 'Error: Could Not open Port')
    else:
  ...
  def cancel(self):
    ...
    self.auto = False
    self.reading = False
    #self.socket.write(struct.pack('<I', 11<<28))                           This is a stop command that will stop the vna during a sweep needed functionality in Code Composer
    self.progressBar.setValue(0)
    self.set_enabled(True)
...
#This is where all the math ends
